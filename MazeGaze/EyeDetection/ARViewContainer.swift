...
class CustomARView: ARView, ARSessionDelegate {
    ...
    private func detectGazePoint(faceAnchor: ARFaceAnchor){
        ...
        // For iPhone and iPad non 10th gen
        ...
        // For iPad 10th gen
        let screenX = transformedLookAtPoint.x / (Float(Device.screenSize.height) / 2) * Float(Device.frameSize.width)
        let screenY = -transformedLookAtPoint.y / (Float(Device.screenSize.width) / 2) * Float(Device.frameSize.height)
